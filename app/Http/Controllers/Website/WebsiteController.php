...
class WebsiteController extends Controller
{
    public function index()
    {
        ...
        return view('frontend.index', compact('banners', 'doctors', 'treatments', 'setting', 'reviews', 'blogs','hospitalData'));
    }
    ...
    public function booking($id, $name)
    {
        ...
        $emergencyData=DB::table('emergency_slot')->where('created_at','=',$current_date)->orderBy('id','desc')->where('doctor_id',$id) ->first();
// if(!$emergencyData){
//  $emergencyData=0;
// }
        ...
        return view('website.appointment_booking', compact('doctor', 'patient_addressess', 'today_timeslots', 'currency', 'setting','insurers','razorpayKey','emergencyData'));
    }
...
public function emergencyText(){
    ...
    $current_date=Carbon::now()->format('Y-m-d');
// echo $current_date;
    // DB::table('emergency_slot')->where('created_at','=',$current_date)->where('1','doctor_id')->orderBy('id','desc')->limit(1)->increment('total_patient');
    $emergencyData=DB::table('emergency_slot')->where('created_at','=',$current_date)->where('doctor_id','1')->orderBy('id','desc')->limit(1)->increment('total_patient');
    ...
    $current_date=Carbon::now()->format('Y-m-d');
        ...
        $data['time'] = 'Emergency Patient';
           
    }

  
    
    //     if($request->select('types')=='emergency'){
    //         $request->validate([
    //             'appointment_for' => 'bail|required',
    //             'illness_information' => 'bail|required',
    //             'patient_name' => 'bail|required',
    ...
    // }
        ...
        if($data['types']=='1'){
            ...
            DB::table('emergency_slot')->where('created_at','=',$current_date)->where('doctor_id',$data['doctor_id'])->orderBy('id','desc')->limit(1)->increment('total_patient');
            $emergencyData=DB::table('emergency_slot')->where('created_at','=',$current_date)->where('doctor_id',$data['doctor_id'])->orderBy('id','desc')->limit(1)->get();
            $data['amount'] = $emergencyData[0]->fee;
        }
    ...
    public function aboutUs()
    {
        ...
        return view('frontend.about.about-us', compact('aboutUs'));
    }
