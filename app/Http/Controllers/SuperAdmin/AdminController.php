...
class AdminController extends Controller
{
    ...
    public function softwarechargepost(Request $request){
        ...
        // echo $price;
        DB::table('software_charge')->where('id',1)->update(['price'=>$price]);
        return redirect()->back()->with('success','successfully update software charge');
     }
    //  careerdetails
    public function careerdetails(){
        $careerData=DB::table('career')->orderBy('id','desc')->get();
        // echo 'hii';
        return view('superAdmin.career',compact('careerData'));
    }
