            ...
            <div class="xxsm:pl-0 xmd:pl-10 xxsm:ml-0 xlg:pl-0 lg:ml-0 lg:pl-0 xlg:ml-5 md:pl-5 md:border-l-2 md:border-white-light xxsm:border-l-0 lg:border-l-0 xl:border-l-2 xl:pl-5">
                <div>
                    ...
                    <div class="flex xl:flex-row xlg:flex-row lg:flex-row xsm:flex-row xxsm:flex-col items-center">
                        <a href="{{ url('booking/'.$doctor['id'].'/'.Str::slug($doctor['name'])) }}" class="font-fira-sans text-white bg-primary hover:bg-primary text-sm text-center py-2.5 px-5 bg-blue-500 px-2">{{__('Make Appointment')}}</a>
                        <a href="{{ url('doctor-profile/'.$doctor['id'].'/'.Str::slug($doctor['name'])) }}" class="font-fira-sans text-primary text-sm font-normal leading-4 underline py-2 ml-3 bg-blue-500 px-2 text-white">{{__('View Profile')}}</a>
                    </div>
                </div>
            </div>
        </div>
