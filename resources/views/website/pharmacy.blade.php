...
<h1 class="text_clr1 text-start text-5xl font-semibold text-white drop-shadow-lg drop-shadow-black " >Pharmacy</h1>
...
<div class="pt-14 border-b border-gray-light mb-10 pb-10">
    ...
    <form id="searchForm" method="post">
        ...
        xxsm:space-y-2 xxsm:p-2">
            ...
            <button type="button" onclick="searchPharmacy()"  class="text-white bg-primary text-center px-6 py-2 text-base font-normal leading-5 font-fira-sans sm:w-32 msm:w-32 xsm:w-32 xxsm:w-32 h-12 bg-blue-500 px-2"><i class="fa-solid fa-magnifying-glass"></i> {{__('Search')}}</button>
        </div>
    </form>
</div>
        ...
        @if ($pharmacies['current_page'] != $pharmacies['last_page'])
            <div class="flex justify-center pt-8 pb-32 2xl:ml-64 xl:ml-72 xlg:ml-64 lg:ml-54 xmd:ml-44 sm:ml-20 xsm:ml-5 xxsm:ml-4">
                <div class="sm:py-3 md:py-0 msm:py-3 xsm:py-3 xxsm:py-3" id="">
                    <button id="more-doctor" type="button" class="text-sm font-normal font-fira-sans leading-4 md:text-sm text-primary border border-primary text-center py-3.5 px-6 " style="background-color: blue;">{{__('View More')}}</button>
                </div>
            </div>
        @endif
    @else
