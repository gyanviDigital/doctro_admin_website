...
<h1 class="text_clr1 text-start text-5xl font-semibold text-white drop-shadow-lg drop-shadow-black " >Doctors</h1>
<a href="{{url('/')}}" class=" border-b-2 border-blue pb-1 text-3xl text_clr2 font-bold" >HOME</a>
    ...
    <div class="pt-14 border-b border-white-light mb-10 pb-10">
        ...
        <form action="{{ url('show-doctors') }}" id="searchForm" method="post">
            @csrf
            <div class="lg:flex place-items-center place-content-center sm:grid place-items-center">
                <div class="relative">
                ...
                <button type="button" onclick="searchDoctor()" class="text-white bg-primary text-center px-6 py-2 text-base font-normal leading-5 font-fira-sans sm:w-32 msm:w-32 xsm:w-32 xxsm:w-32 h-12" style="background-color: #09332f;"><i
            ...
            @if ($doctors['current_page'] != $doctors['last_page'])
                <div
                    ...
                    <div class="sm:py-3 md:py-0 msm:py-3 xsm:py-3 xxsm:py-3" id="">
                        <button id="more-doctor" type="button"  class="text-sm font-normal font-fira-sans leading-4 md:text-sm text-primary border border-primary text-center py-3.5 px-6 bg-blue-500 px-2 ">{{ __('View More') }}</button>
                    </div>
                </div>
            @endif
        @else
