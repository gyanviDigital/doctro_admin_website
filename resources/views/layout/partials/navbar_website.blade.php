<nav class="border-gray-200 sm:px-4 py-2.5 rounded border-b border-slate"   id="navbar_website" style="display: none;" > 
...
<div class="xxxxxxxxx" style="position: absolute; width:100%; display:flex; justify-content:space-between;">
    <div class=""></div>
 <div class="flex items-center md:order-2 place-self-end">
                ...
                <div id="allLanguages" style="z-index: 50; position:absolute; margin-top:190px;" class="list-none divide-y hidden divide-gray-100 rounded-lg shadow bg-white" id="language-dropdown-menu">
                    <ul class="" style="" role="none">
                        @foreach ($languages as $language)
                        <li>
                            <a href="{{ url('select_language',$language->id) }}" class="block px-4  py-2 text-sm text-black-700" role="menuitem">
                                <div class="inline-flex items-center">
                ...
                <div class="flex items-center relative">
                    ...
                    @if (auth()->check())
                    <button id="profileBtn" data-dropdown-toggle="dropdownAvatarName" class="flex items-center text-sm font-medium text-gray-900 rounded-full hover:text-blue-600 dark:hover:text-blue-500 md:mr-0 dark:text-white" type="button">
                        <span class="sr-only">Open user menu</span>
                        <img class="w-8 h-8 mr-2 rounded-full" src="{{ url('images/upload/'.auth()->user()->image) }}" alt="user photo">
                        <div class="text-gray-500">{{ auth()->user()->name }}</div>
                        <svg class="w-4 h-4 mx-1.5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
