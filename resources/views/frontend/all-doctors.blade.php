@extends('layout2.index',['title'=>'Doctor'])
@section('main')
<main>
    @include('layout2.page-name',[$pagename='All-Doctors'])
<!-- Main Container -->
<div class="container mx-auto px-6 py-20">
    <!-- Title Section -->
    <div class="text-center mb-12">
      <h2 class="text-3xl md:text-4xl font-bold text-gray-800">
        Discover the <strong class="text-blue-600">online</strong> appointment!
      </h2>
      <p class="mt-4 text-gray-600 max-w-2xl mx-auto">
        Usu habeo equidem sanctus no. Suas summo id sed, erat erant oporteat cu pri. In eum omnes molestie. Sed ad debet scaevola, ne mel.
      </p>
    </div>

    <!-- Features Row -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
      <!-- Feature 1 -->
  
         <div class="bg-white p-8 rounded-lg shadow-lg">
        <div id="icon_1" class="text-center">
            <div class="w-cover grid place-items-center">
        <img src="{{url('image/descover/img1.png')}}" alt="" class="self-center w-28">
        </div>
          <h3 class="text-xl font-semibold text-gray-800 mb-4">Find a Doctor</h3>
          <p class="text-gray-600">
            Usu habeo equidem sanctus no. Suas summo id sed, erat erant oporteat cu pri. In eum omnes molestie.
          </p>
        </div>
      </div>

      <!-- Feature 2 -->
      <div class="bg-white p-8 rounded-lg shadow-lg">
        <div id="icon_2" class="text-center">
        <div class="w-cover grid place-items-center">
        <img src="{{url('image/descover/img3.png')}}" alt="" class="self-center w-28">
        </div>
          <h3 class="text-xl font-semibold text-gray-800 mb-4">Find a Hospitals</h3>
          <p class="text-gray-600">
            Usu habeo equidem sanctus no. Suas summo id sed, erat erant oporteat cu pri. In eum omnes molestie.
          </p>
        </div>
      </div>

      <!-- Feature 3 -->
      <div class="bg-white p-8 rounded-lg shadow-lg">
        <div id="icon_3" class="text-center">
        <div class="w-cover grid place-items-center">
        <img src="{{url('image/descover/clinic.jpg')}}" alt="" class="self-center w-28">
        </div>
          <h3 class="text-xl font-semibold text-gray-800 mb-4">Find a Clinic</h3>
          <p class="text-gray-600">
            Usu habeo equidem sanctus no. Suas summo id sed, erat erant oporteat cu pri. In eum omnes molestie.
          </p>
        </div>
      </div>
    </div>
  </div>
  <!-- alll doctors -->
   
  <section>
    <!--main member-->
 <div data-aos="fade-up" class="p_a latest-main-box grid place-items-center">
    <h1 class="text-5xl font-bold text_clr2 text-start">Top Hospitals</h1>
    <div data-aos="fade-up" class="w-auto justify-between text-center mt-8">
 <!--new-->
 <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    <div data-aos="fade-up" class="inline-block w-80  h-auto m-4 p-2 hover3 shadow-lg shadow-blue-600">
        <div data-aos="fade-up" class="w-auto overflow-hidden h-72 grid place-items-center">
            <img src="{{asset($web_logo)}}" alt="" class="hover1  w-auto h-72 ">
        </div>
        <div data-aos="fade-up" class="w-auto h-18 text-start overflow-hidden ">
            <strong class="text-gray-400 uppercase" >Specialization</strong> 
            <h3 class="text-3xl font-semibold flex place-items-center place-content-start gap-4">Username
                 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="green" class="bi bi-check-circle-fill" viewBox="0 0 16 16">
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
</svg></h3>
<h3 class="font-semibold text-xl">Address</h3>
            <p class="text-sm text-start">Id placerat tacimates definitionem sea, prima quidam vim no. Duo nobis persecuti cuodo....</p>
           <!--review-->
            <div class="flex place-items-center place-content-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-fill" viewBox="0 0 16 16">
  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
</svg>
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FFC000" class="bi bi-star-half" viewBox="0 0 16 16">
  <path d="M5.354 5.119 7.538.792A.52.52 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.54.54 0 0 1 16 6.32a.55.55 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.5.5 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.6.6 0 0 1 .085-.302.51.51 0 0 1 .37-.245zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.56.56 0 0 1 .162-.505l2.907-2.77-4.052-.576a.53.53 0 0 1-.393-.288L8.001 2.223 8 2.226z"/>
</svg> 
     </div>
     <hr class="mt-2">
     <div class="flex justify-between mt-4">
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr2 text-white font-semibold">View Profile</button></a>
        <a href="{{url('/')}}"><button class="py-2 px-4 rounded border border-white bg_clr1 text-white font-semibold">Book Now</button></a>
     </div>
        </div>
    </div>
    <!--new box-->
    </div>
   
 </div>
 

  </section>
</main>
@endsection